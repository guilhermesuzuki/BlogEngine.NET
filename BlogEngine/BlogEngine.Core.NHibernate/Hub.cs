...
namespace BlogEngine.Core.NHibernate
{
    static class Hub
    {
        /// <summary>
        ...
        /// <summary>
        /// Creates a Session Factory for NHibernate ORM Operations
        /// </summary>
        /// <returns></returns>
        public static ISessionFactory GetSessionFactory(string connStringName)
        {
            //tries to find the connection string
            connStringName = ConfigurationManager.ConnectionStrings[connStringName] == null ? "BlogEngine" : connStringName;

            //key for accessing cache information
            string key = $"nhibernate-{connStringName.ToLower()}-sessionfactory";
            ...
            if (HttpRuntime.Cache[key] == null)
            {
                lock (SyncRoot)
                {
                    if (HttpRuntime.Cache[key] == null)
                    {
                        var c = ConfigurationManager.ConnectionStrings[connStringName];
                        IPersistenceConfigurer d = MsSqlConfiguration.MsSql2012.ConnectionString(c.ConnectionString).ShowSql();

                        //gets the provider name
                        if (string.IsNullOrWhiteSpace(c.ProviderName) == false)
                        {
                            var pname = c.ProviderName.Trim().ToLower();

                            switch (pname)
                            {
                                case "system.data.oracleclient":
                                    d = OracleClientConfiguration.Oracle10.ConnectionString(c.ConnectionString).ShowSql();
                                    break;
                                case "system.data.sqlite":
                                    d = SQLiteConfiguration.Standard.ConnectionString(c.ConnectionString).ShowSql();
                                    break;
                                case "mysql.data.mysqlclient":
                                    d = MySQLConfiguration.Standard.ConnectionString(c.ConnectionString).ShowSql();
                                    break;
                            }
                        }

                        var a = typeof(Hub).Assembly;

                        //session factory was built
                        ...
                        //adds new session in cache
                        HttpRuntime.Cache.Insert(key, _sessionFactory, null, Cache.NoAbsoluteExpiration, Cache.NoSlidingExpiration);
                    }
                }
            }

            //returns the object
            return HttpRuntime.Cache[key] as ISessionFactory;
        }
