...
{
    public class EFMembershipProvider: DbMembershipProvider
    {
        /// <summary>
        /// EF DataContext
        /// </summary>
        ...
        public override bool ChangePassword(string username, string oldPassword, string newPassword)
        {
            ...
            using (var ts = new TransactionScope(TransactionScopeOption.Required) { })
            {
                ...
                {
                    var actualPassword = dbContext.Users.FirstOrDefault(user => user.BlogID == Blog.CurrentInstance.BlogId && user.UserName.ToLower() == username.ToLower())?.Password;
                    ...
                    {
                        var toupdate = dbContext.Users.FirstOrDefault(user => user.BlogID == Blog.CurrentInstance.BlogId && user.UserName.ToLower() == username.ToLower());
                        if (toupdate != null)
                        {
        ...
        public override bool DeleteUser(string username, bool deleteAllRelatedData)
        {
            using (var ts = new TransactionScope(TransactionScopeOption.Required) { })
            {
                try
                {
                    using (var dbContext = this.GetDbContext())
                    {
                        var todelete = dbContext.Users.FirstOrDefault(user => user.BlogID == Blog.CurrentInstance.BlogId && user.UserName.ToLower() == username.ToLower());
        ...
        public override MembershipUserCollection GetAllUsers(int pageIndex, int pageSize, out int totalRecords)
        {
            ...
            using (var ts = new TransactionScope(TransactionScopeOption.Required) { })
            {
                using (var dbContext = this.GetDbContext())
                {
                    ...
                    {
                        var allusersfromblog = dbContext.Users.Where(user => user.BlogID == Blog.CurrentInstance.BlogId).ToList();
        ...
        public override MembershipUser GetUser(string username, bool userIsOnline)
        {
            using (var dbContext = this.GetDbContext())
            {
                var user = dbContext.Users.FirstOrDefault(u => u.BlogID == Blog.CurrentInstance.BlogId && u.UserName.ToLower() == username.ToLower());
        ...
        public override string GetUserNameByEmail(string email)
        {
            ...
            using (var dbContext = this.GetDbContext())
            {
                var user = dbContext.Users.FirstOrDefault(i => i.BlogID == Blog.CurrentInstance.BlogId && i.EmailAddress.ToLower() == email.ToLower());
