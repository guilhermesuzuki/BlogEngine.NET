...
var common = {
    ...
    enabling: {
        ...
        tooltipster: function (elm) { common.enabling.toolTipster(elm); },
        /*function that creates tooltipster for an element*/
        ...
        yoxview: function (selector) {
            $(selector).find(".yoxview-activate").each(
                function () {
                    var elm = $(this);
                    elm.yoxview({
                        allowedUrls: /\/files\/(.*)\?size=(large|original|medium|small)$/i,
                        lang: $("html").attr("lang"),
    ...
    yoxview: function (selector) {
        $(selector).find("img[src^='/files/']").each(
            function () {
                var img = $(this);
...
/*initializes video controls using plyr*/
$(document).ready(function () { 
    if (Plyr) var player = new Plyr("video");
    });